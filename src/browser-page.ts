...
function providerLabel(provider){const access=provider.methods.map(method=>method.label).join(' / ');return provider.name+(access?' — '+access:'')+(provider.configured?' · connected':'')}
...
const selectedProvider=assistant.auth?.providerId||assistant.selected?.provider||$('provider').value||assistant.providers[0]?.id||'';$('provider').replaceChildren();for(const provider of assistant.providers)addOption($('provider'),provider.id,providerLabel(provider),selectedProvider);updateMethods();
